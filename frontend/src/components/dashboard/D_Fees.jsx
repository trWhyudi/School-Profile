...
const D_Fees = () => {
  ...
  const fetchStudents = async () => {
    try {
      const res = await axios.get(
        "http://localhost:5050/api/v1/student/get-all-student",
  ...
  const fetchFees = async () => {
    try {
      const res = await axios.get(
        "http://localhost:5050/api/v1/fee/get-all-fee",
        { withCredentials: true }
      );
  ...
  const payload = {
    ...formData,
    paymentDate:
      formData.status === "Dibayar" && formData.paymentDate
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (updateId) {
        await axios.put(
          `http://localhost:5050/api/v1/fee/update-fee/${updateId}`,
          payload,
          { withCredentials: true }
        );
        ...
        await axios.post(
          "http://localhost:5050/api/v1/fee/create-fee",
          payload,
          { withCredentials: true }
        );
  ...
  const handleDelete = async (id) => {
    ...
    if (resp.isConfirmed) {
      try {
        await axios.delete(
          `http://localhost:5050/api/v1/fee/delete-fee/${id}`,
          { withCredentials: true }
        );
              ...
              {fees.length > 0 ? (
                fees.map((f, idx) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500">
                      {f.paymentDate
                        ? new Date(f.paymentDate).toLocaleDateString("id-ID")
                        : "-"}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-500">
