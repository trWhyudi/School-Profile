import React from 'react'
import { Link } from 'react-router-dom';
import { FaFacebookSquare, FaYoutube } from "react-icons/fa"
import { FaInstagram } from "react-icons/fa";

const Footer = () => {
  return (
    <div>
      <footer className='bg-gray-900 text-white pt-16 pb-8 px-6'>
        <div className='max-w-[1440px] mx-auto grid md:grid-cols-4 gap-10'>
          <div className=''>
            <h2 className='text-2xl md:text-3xl font-bold text-cyan-500'>SMAN 1 Cibitung</h2>
            <p className='text-gray-300 text-lg font-serif font-medium mt-2'>Lorem ipsum dolor sit amet consectetur, adipisicing elit. Rem, dolor!</p>
          </div>
          <div>
            <h3 className='text-lg font-semibold mb-3 md:text-xl font-sans'>Tautan Cepat</h3>
            <ul className='space-y-3 text-gray-300 text-sm md:text-lg'>
              <li className='hover:text-cyan-500 cursor-pointer'>
                <Link to={"/"}>Beranda</Link>
              </li>
              <li className='hover:text-cyan-500 cursor-pointer'>
                <Link to={"/about"}>Tentang Kami</Link>
              </li>
              <li className='hover:text-cyan-500 cursor-pointer'>
                <Link to={"/services"}>Layanan</Link>
              </li>
              <li className='hover:text-cyan-500 cursor-pointer'>
                <Link to={"/contact"}>Kontak</Link>
              </li>
              <li className='hover:text-cyan-500 cursor-pointer'>
                <Link to={"/policy"}>Kebijakan Privasi</Link>
              </li>
            </ul>
          </div>
          <div>
            <h3 className='text-lg font-semibold mb-3 md:text-xl font-sans'>Support</h3>
            <ul className='space-y-3 text-gray-300 text-sm md:text-lg'>
              <li className='hover:text-cyan-500 cursor-pointer'>
                <Link to={"/faq"}>FAQ</Link>
              </li>
              <li className='hover:text-cyan-500 cursor-pointer'>
                <Link to={"/policy"}>Kebijakan Privasi</Link>
              </li>
              <li className='hover:text-cyan-500 cursor-pointer'>
                <Link to={"/terms"}>Syarat dan Ketentuan</Link>
              </li>
              <li className='hover:text-cyan-500 cursor-pointer'>
                <Link to={"/help"}>Pusat Bantuan</Link>
              </li>
            </ul>
          </div>
          <div>
            <h3 className='text-lg font-semibold mb-3'>Ikuti Kami</h3>
            <div className='flex gap-4 cursor-pointer items-center'>
              <Link to={""}>
                <FaFacebookSquare size={22} className='hover:text-blue-600'/>
              </Link>
              <Link to={""}>
                <FaYoutube size={22} className='hover:text-red-600'/>
              </Link>
              <Link to={""}>
                <FaInstagram size={22} className='hover:text-pink-600'/>
              </Link>
            </div>
          </div>
        </div>
      </footer>
    </div>
  )
}

export default Footer